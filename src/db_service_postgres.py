...
class PostgresDatabaseService(DatabaseServiceBase):
    ...
    def get_player_team_assignment(self, name: str, year: int) -> Optional[str]:
        ...
        with self.get_connection() as conn:
            c = conn.cursor()
            c.execute('SELECT team FROM player_teams WHERE name = %s AND year = %s', (name, year))
            result = c.fetchone()
            return result[0] if result else None
    ...
    def get_team_assignments_by_year(self, year: int) -> List[Dict]:
        ...
        with self.get_connection() as conn:
            query = '''
                ...
                LEFT JOIN handicaps h ON pt.name = h.name AND h.year = %s
                WHERE pt.year = %s
    ...
    def delete_player_team(self, name: str, year: int) -> bool:
        ...
        with self.get_connection() as conn:
            c = conn.cursor()
            c.execute('DELETE FROM player_teams WHERE name = %s AND year = %s', (name, year))
            return c.rowcount > 0
    ...
    def get_pairing_chat_session(self, email: str, year: int) -> Optional[Dict]:
        ...
        with self.get_connection() as conn:
            c = conn.cursor()
            c.execute('SELECT conversation, messages FROM pairing_chat_sessions WHERE email = %s AND year = %s',
                      (email, year))
            row = c.fetchone()
            if not row:
                return None
    ...
    def delete_pairing_chat_session(self, email: str, year: int) -> bool:
        ...
        with self.get_connection() as conn:
            c = conn.cursor()
            c.execute('DELETE FROM pairing_chat_sessions WHERE email = %s AND year = %s', (email, year))
            return c.rowcount > 0
